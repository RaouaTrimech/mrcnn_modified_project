...
from mrcnn_mobilenet.model_init import MaskRCNN
from mrcnn_mobilenet.model_init import load_image_gt
from mrcnn_mobilenet.model_init import mold_image
...
class KangarooDataset(Dataset):
    ...
    def load_dataset(self, dataset_dir, is_train=True):
        ...
        for filename in sorted(os.listdir(images)):
            ...
            self.add_image('dataset', image_id=img_id, path=img_path, annotation=ann_path, class_name="kangaroo")
...
class KangarooConfig(Config):
    ...
    NUM_CLASSES = 1 + 1
    LEARNING_RATE = 0.01
    STEPS_PER_EPOCH = 5 #131 # 5 to test
...
if Train:
    ...
    model.train(train_set, test_set, learning_rate=config.LEARNING_RATE, epochs=2, layers='heads') # we put 2 epoch just to test
